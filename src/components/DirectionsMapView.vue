...
export default {
  ...
  mounted() {
    ...
    EventBus.$on("routes-data", routes => {
      this.map = new google.maps.Map(this.$refs["map"],{
        center: new google.maps.LatLng(21.0181, -101.258),
        zoom: 15,
      ...
      routes.forEach(({origin, destination, distance, duration, color}) => {
        directionsService.route({
          ...
          if(status==="OK"){
            ...
            this.createPolylineWith(origin, overviewPath[0], color);
            this.createPolylineWith(destination, overviewPath[overviewPath.length-1], color);
          }
        });
      });
    });
  ...
  methods: {
    ...
    createPolylineWith(location, overview, color){
      new google.maps.Polyline({
        path: [
          {lat: location.lat, lng: location.lng},
          {lat: overview.lat(), lng: overview.lng()},
        ],
        strokeColor: color,
        strokeOpacity: 1,
        strokeWeight: 8,
